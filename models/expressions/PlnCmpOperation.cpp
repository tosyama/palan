...
PlnExpression* PlnCmpOperation::create(PlnExpression* l, PlnExpression* r, PlnCmpType cmp_type)
{
	...
	if (!is_l_num_lit || !is_r_num_lit) {
		...
		if (migrate)
			...
			return new PlnCmpOperation2(l, r, cmp_type);
	}
...
//vvvvvvvvvv Old
PlnCmpOperation2::PlnCmpOperation2(PlnExpression* l, PlnExpression* r, PlnCmpType cmp_type)
...
PlnCmpOperation2::~PlnCmpOperation2()
{
	delete l;
	delete r;
}

void PlnCmpOperation2::finish(PlnDataAllocator& da, PlnScopeInfo& si)
{
	BOOST_ASSERT(data_places.size() <= 1);
...
void PlnCmpOperation2::gen(PlnGenerator& g)
{
